...
public class DatabaseSequenceFilterTest extends TestCase
{
    ...
    public void testSkipOrdering() throws Exception{
        String[] expectedNoFilter = {"A","B","C","D","E",};

        HypersonicEnvironment.executeDdlFile(TestUtils.getFile("sql/hypersonic_cyclic.sql"),
                _jdbcConnection);
        IDatabaseConnection connection = new DatabaseConnection(_jdbcConnection);
        connection.getConfig().setProperty(DatabaseConfig.FEATURE_SKIP_TABLE_ORDERING, Boolean.TRUE);
        ...
        ITableFilter filter = new DatabaseSequenceFilter(connection);
        IDataSet filteredDataSet = new FilteredDataSet(filter, databaseDataset);
        assertTrue(Arrays.deepEquals(expectedNoFilter, filteredDataSet.getTableNames()));
    }

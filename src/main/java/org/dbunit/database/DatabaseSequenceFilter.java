 ...
 * @version $Revision: 1086 $ $Date: 2009-10-30 11:50:38 +0100 (p, 30 okt 2009) $
...
public class DatabaseSequenceFilter extends SequenceTableFilter
{
            ...
            // not sure why this throws DataSetException ? - ENP
    {
        ...
        for (Iterator iterator = dependencies.values().iterator(); iterator.hasNext();) {
            ...
            if(info.hasCycles()){
                boolean skipOrdering = ((Boolean)connection.getConfig().getProperty(DatabaseConfig.FEATURE_SKIP_TABLE_ORDERING)).booleanValue();
                if(skipOrdering){
                    return tableNames;
                } else {
                    throw new CyclicTablesDependencyException(info.getTableName(), info.getAllTableDependsOn());
                }
            }
        }
